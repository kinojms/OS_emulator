...
private:
    ...
    int memorySize; // total bytes allocated for this process
    std::unordered_map<std::string, uint16_t> symbolTable; // variable name -> value
    std::unordered_map<uint32_t, uint16_t> memory; // address -> value
...
public:
    ...
    Process(int pid, const std::string& name = "", int memorySize); // Add name and memorySize to constructor

...
Process::Process(int pid, const std::string& name, int memorySize) : pid(pid), processName(name), memorySize(memorySize) {
    if (processName.empty())
        processName = "process_" + std::to_string(pid);
    filename = processName + ".txt";
}

// READ: retrieves value at address, stores in var
uint16_t Process::READ(const std::string& var, uint32_t address) {
    // Check address bounds
    if (address >= static_cast<uint32_t>(memorySize)) {
        logs.push_back("Access violation: READ at address 0x" + 
            static_cast<std::ostringstream&&>(std::ostringstream() << std::hex << address).str());
        isFinished = true;
        throw std::runtime_error("Access violation: READ");
    }
    // Symbol table limit
    if (symbolTable.size() >= 32 && symbolTable.find(var) == symbolTable.end()) {
        logs.push_back("Symbol table full. Variable '" + var + "' not created.");
        return 0;
    }
    uint16_t value = 0;
    if (memory.count(address)) value = memory[address];
    symbolTable[var] = value;
    logs.push_back("READ " + var + " 0x" + 
        static_cast<std::ostringstream&&>(std::ostringstream() << std::hex << address).str() + 
        " => " + std::to_string(value));
    return value;
}
...
void Process::WRITE(uint32_t address, uint16_t value) {
    if (address >= static_cast<uint32_t>(memorySize)) {
        logs.push_back("Access violation: WRITE at address 0x" + 
            static_cast<std::ostringstream&&>(std::ostringstream() << std::hex << address).str());
        isFinished = true;
        throw std::runtime_error("Access violation: WRITE");
    }
    value = std::clamp<uint16_t>(value, 0, std::numeric_limits<uint16_t>::max());
    memory[address] = value;
    logs.push_back("WRITE 0x" + 
        static_cast<std::ostringstream&&>(std::ostringstream() << std::hex << address).str() + 
        " " + std::to_string(value));
}
...
void Process::parseInstruction(const std::string& instructionLine) {
